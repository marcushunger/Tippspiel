...
//		TypedQuery<User> query = manager.createQuery("select p from User p WHERE p.id = :id", User.class);
...
//		p.setPassphrase_md5("1d028378e12ca6bdafa3b8b21bc5a9ea".toCharArray());
	...
	public List<User> list() throws DaoException {
		return manager.createQuery("select p from User p ", User.class).getResultList(); //Person geht auf die Klasse Person kein sql mehr
	}

	@Override
	public User login(String email, String password) throws DaoException {
	
		//TODO Passworthash nehmen aktuell einfacher Passwortvergleich ohne verschlüsselung
		//JPA hat wohl keine sha2 funktion - 
		//TypedQuery<Person> queryLogin = manager.createQuery("select p from Person p WHERE p.email = :email and p.passphrase_sha2_salted = sha2(CONCAT(:password, p.salt), 512)", Person.class);
		TypedQuery<User> queryLogin = manager.createQuery("select p from User p WHERE p.email = :email and p.passphrase = :password", User.class);
